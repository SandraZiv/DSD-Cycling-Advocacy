...
def _get_longest_paths(nodes, graph, maxnum=5, delta=10):
    ...
    longest_paths = sorted(_gen_paths_distances(), reverse=True)
    max_lenght = max([y for (y, x) in longest_paths])
    return [x for (y, x) in longest_paths if abs(y - max_lenght) < delta][:maxnum]
...
def _yield_ridge_vertices(vor, geometry, dist=False):
    ...
    for x, y in vor.ridge_vertices:
        ...
        if True and point1.within(geometry) and point2.within(geometry):
            if dist:
